...
git checkout main
git pull
git checkout -b claude/fase6-cartografia-cholula

mkdir -p 'data'
cat > 'data/secciones_san_pedro_cholula.geojson' <<'SIAPE_F6B_EOF_1_MARK'
{
  "type": "FeatureCollection",
  "metadata": {
    "_nota": "Secciones electorales REALES de San Pedro Cholula, Puebla (municipio IFE 21141, clave INEGI 21140). Fuente original: cartografía pública del INE (Marco Geográfico Electoral), redistritación 2021. Obtenido vía el espejo académico github.com/emagar/mxDistritos (MIT), no directamente del INE (ese dominio está bloqueado para este agente al momento de generar el archivo). Geometría tomada de shape.__geo_interface__ de pyshp (respeta la orientación de anillos OGC) y reproyectada de UTM Zone 14N (EPSG:32614) a WGS84 (EPSG:4326).",
    "secciones_esperadas_no_incluidas": [
      2787,
      2788,
      2789,
      2790,
    ...
    "nota_secciones_faltantes": "Estas 5 secciones existen en el catálogo de equivalencias seccionales (tablaEquivalenciasSeccionalesDesde1994.csv) como parte de San Pedro Cholula para la redistritación 2021, pero no tienen geometría en este shapefile — verificar con el INE si se requiere el conjunto completo de 51.",
    "fuente": "https://github.com/emagar/mxDistritos/tree/master/mapasComparados/sec/shp/sec2021/pue",
    "generado": "2026-08-12"
  },
  "features": [
    {
...
SIAPE_F6B_EOF_1_MARK

cat > 'requirements.txt' <<'SIAPE_F6B_EOF_2_MARK'
anthropic>=0.40
SQLAlchemy>=2.0
alembic>=1.13
psycopg2-binary>=2.9
...
# Solo para scripts/extraer_secciones_shapefile.py (herramienta de carga de
# cartografía geo, Fase 5/6 — no se usan en el resto de la aplicación)
pyshp>=2.3
pyproj>=3.6
SIAPE_F6B_EOF_2_MARK

mkdir -p 'scripts'
cat > 'scripts/cargar_secciones_electorales.py' <<'SIAPE_F6B_EOF_3_MARK'
#!/usr/bin/env python
...
if __name__ == "__main__":
    main()
SIAPE_F6B_EOF_3_MARK

mkdir -p 'scripts'
cat > 'scripts/extraer_secciones_shapefile.py' <<'SIAPE_F6B_EOF_4_MARK'
#!/usr/bin/env python
...
if __name__ == "__main__":
    main()
SIAPE_F6B_EOF_4_MARK

mkdir -p 'tests'
cat > 'tests/test_cargar_secciones_electorales.py' <<'SIAPE_F6B_EOF_5_MARK'
...
SIAPE_F6B_EOF_5_MARK

git add -A
git commit -m "Implement Fase 6.2: cartografia real de San Pedro Cholula (INE)"
git push -u origin claude/fase6-cartografia-cholula
echo "LISTO"